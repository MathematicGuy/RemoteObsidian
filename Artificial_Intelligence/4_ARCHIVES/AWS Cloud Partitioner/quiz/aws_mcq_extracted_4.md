...
- [ ] B. Grant users only the specific permissions necessary to perform their required tasks.
...
- [ ] D. Implementing Cost Allocation Tags
...
- [ ] C. Inbound data transfer (data into AWS) is generally free, while outbound data transfer (data out to the internet) is charged.
...
- [ ] B. Consolidated Billing (AWS Organizations)
...
- [ ] B. The user lacks the appropriate cross-account IAM permissions/permission sets to create resources in the production account.
...
- [ ] A. Enable Amazon Inspector to automatically scan container images (Amazon ECR).
...
- [ ] B. OS patching and installing antivirus software on the instance.
...
- [ ] C. Data transfer from the Internet to AWS (Inbound Data Transfer).
...
- [ ] B. They cannot attribute spend or identify wasted resources.
...
- [ ] C. Create an IAM group and attach policies to the group
...
- [ ] A. Use IAM policies
- [ ] B. Use AWS KMS with automatic key rotation
- [ ] C. Use AWS Shield
...
- [ ] B. Create an IAM role with the required S3 permissions and attach it to the EC2 instance.
...
- [ ] B. It ensures high availability and fault tolerance by distributing traffic across multiple Availability Zones.
...
- [ ] C. It improves availability and distributes traffic across targets
...
- [ ] B. Data is decrypted after being sent
...
- [ ] C. Configuring AWS Budgets and alerts
...
- [ ] B. Storage class used and the total amount of data stored
...
- [ ] C. IAM Users have long-term login credentials, while IAM Roles use temporary security credentials.
...
- [ ] C. AWS handles the management of the underlying OS, network configuration, and platform software for managed services
...
- [ ] C. The operating system and applications running on the instance
...
- [ ] A. Compute, Managed Services, Storage, Network Traffic & Data Transfer, Misc
...
- [ ] A. Compute Cost = Runtime × Bandwidth × Users
...
- [ ] C. Configuring client-side data encryption
...
- [ ] D. There is no automatic rotation; it must be done manually
...
- [ ] B. In the same Availability Zone.
...
- [ ] A. Storage Class Tiers
- [ ] B. PUT/GET Request Volume
- [ ] C. Number of IAM Users accessing the bucket
...
- [ ] C. Utilizing batch inference jobs.
...
- [ ] B. Apply User-Defined Cost Allocation Tags to resources.
...
- [ ] B. AWS Config Rules
- [ ] C. Service Control Policies (SCP)
- [ ] D. AWS Security Hub
...
- [ ] B. Managing cost allocation tags to track expenses by department.
...
- [ ] B. Invisible costs like uncleaned storage and unmodeled data transfer
...
- [ ] C. Using Batch Transform jobs instead of real-time endpoints
...
- [ ] C. AWS WAF with a Web ACL attached to the ALB
- [ ] D. Amazon GuardDuty
...
- [ ] B. It natively supports automatic rotation of secrets on a schedule using a Lambda function.
...
- [ ] B. Patching the guest operating system and the application software.
...
- [ ] D. Instance Type, Instance Amount, and Runtime
...
- [ ] D. Revenue is unpredictable and volatile
...
- [ ] C. NAT Gateway incurs both a per-hour service charge and a per-GB data processing charge.
...
- [ ] B. Real-Time SageMaker endpoints run 24/7 whether they serve requests or not, leading to idle waste.
...
- [ ] B. Symmetric keys use a single key for encryption and decryption, while asymmetric keys use a public/private key pair.
...
- [ ] B. AWS Security Hub & Amazon EventBridge
...
- [ ] A. Amazon S3 -> Amazon Macie (analyze sensitive data) -> Amazon EventBridge (notify & integrate)
...
- [ ] A. Customer managed KMS key that you create and control
...
- [ ] A. ACM manages SSL/TLS certificates for securing network connections, while KMS manages encryption keys for protecting data at rest and in transit
...
- [ ] C. Maintaining the physical hardware in data centers
...
- [ ] C. Enable Multi-Factor Authentication for the root account and critical IAM users
...
- [ ] B. AWS Cost Explorer + AWS Budgets
