...
- [ ] B. S3 Standard
- [x] C. S3 Glacier Deep Archive
- [ ] D. S3 One Zone-IA

Q4. What is AWS EFS?
- [x] A. AWS Elastic File System
...
- [x] A. AWS Quantum Ledger Database
- [ ] B. AWS Neptune
- [ ] C. Amazon DocumentDB
- [ ] D. AWS ElastiCache
...
- [x] C. Improves read performance of NoSQL data
...
- [x] B. Amazon Elastic File System (Amazon EFS)
...
- [x] D. Provisioned IOPS SSD (io1/io2)
...
- [x] A. It cannot span across multiple Availability Zones
...
- [x] C. Migrate the workload to Amazon EC2 or AWS Fargate.
...
- [x] B. You cannot; EBS volumes are tied to a specific AZ. You must restore the data from a snapshot.
...
- [x] B. To provide a preconfigured template to launch EC2 instances
...
- [x] C. It is automatically deleted.
...
- [x] B. The root volume must be an encrypted Amazon EBS volume.
...
- [x] A. Adding more instances to handle load
...
- [x] C. Executes code in response to events
...
- [x] C. Both are only compatible with Linux-based AMIs.
...
- [x] B. Auto Scaling Group (ASG) and Elastic Load Balancing (ELB)
...
- [x] B. Amazon S3 Glacier Deep Archive
- [ ] C. Amazon S3 Glacier Flexible Retrieval
- [ ] D. Amazon S3 Intelligent-Tiering
...
- [ ] A. Cold HDD (sc1)
- [x] B. Throughput Optimized HDD (st1)
...
- [x] C. S3 Cross-Region Replication (CRR) on the bucket
...
- [x] D. Transition objects to S3 Standard-IA after 90 days and keep them there
...
- [x] B. Use Target Tracking Scaling
...
- [ ] B. EC2 + Auto Scaling
- [ ] C. EC2 + Load Balancer
- [x] D. EC2 + Load Balancer + Auto Scaling
...
- [x] C. Create an AMI and use it in a Launch Template
...
- [x] C. Use Amazon EFS and mount it to all instances
...
- [x] A. Amazon S3 Standard with a lifecycle policy transitioning to Glacier
...
- [x] B. Amazon EFS is a network file system
...
- [x] B. Use Amazon EFS and mount it on all EC2 instances
...
- [x] B. One instance (except Multi-Attach)
- [ ] C. None
- [ ] D. Unlimited
...
- [x] C. EBS usually attaches to one instance at a time
...
- [x] C. Amazon EC2 Auto Scaling and Elastic Load Balancing (ELB)
...
- [x] B. Very High Durability and Very High Throughput
...
- [x] B. Distributed Storage System (Max Scale)
- [ ] C. Block Storage
- [ ] D. Local Storage
...
- [x] B. They are billed at the full On-Demand rate for the reserved capacity
...
- [x] C. Performing hardware maintenance in the AWS facilities that run the AWS Cloud
...
- [x] B. Use the right combination of storage classes based on different use cases
...
- [x] C. Block Storage — because of its extremely fast performance and support for high-transaction workloads
...
- [x] C. In the same AZ as the EC2 instance
...
- [x] C. EBS is locked to a single AZ, while EFS data is stored redundantly across multiple AZs.
...
- [x] B. It scales automatically as you add or remove files
...
- [x] C. Writing and maintaining the function code and its dependencies
...
- [x] A. Lambda executes event-driven code and automatically manages the infrastructure, while EC2 requires users to manage and maintain virtual machines.
...
- [x] B. Vertical scaling is restricted by hardware limits of a single machine, while horizontal scaling is theoretically limitless by adding more nodes.
...
- [x] C. It packs instances close together in a single Availability Zone to achieve low-latency, high-throughput networking.
...
- [x] B. It uses a flat address space where each object has a unique identifier.
...
- [x] C. Objects are moved to Glacier after 90 days and automatically deleted after 365 days
