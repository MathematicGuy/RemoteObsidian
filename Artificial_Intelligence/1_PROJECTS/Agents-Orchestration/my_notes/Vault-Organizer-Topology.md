...
## 5. Dynamic Multi-Vault Adaptive Heuristics (Next Milestone)
Making the organization pipeline adaptive across multiple independent Obsidian vaults, dynamically adjusting heuristic classification rules and frontmatter schemas based on each vault's unique note styles, structural taxonomies, and user preferences.

### Core Dependencies

1. **Dynamic Heuristics Configuration Loader**:
   - Decouple the current hardcoded `heuristic_rules.py` rules.
   - Design a structured schema (JSON or YAML) for localized rules stored inside each vault's administrative folder: `<Obsidian-Vault-Name>/.obsidian/vault_rules.json`.
   - Update `organize.py` to dynamically load these rules at scan time based on the active target vault.

2. **User Note Style & Taxonomic Profiler**:
   - A subagent routine that analyzes a sample batch of existing sorted files in the vault to infer:
     - The existing category directory structures and nesting depths.
     - Frontmatter styling choices (e.g., tag formats, key-value mappings).
     - Style and key themes of notes.

3. **Dynamic Rule Synthesizer (Rule Generator)**:
   - An autonomous subagent that auto-generates custom regex patterns and keyword matches from the vault's taxonomic profile, compiling them into `<Obsidian-Vault-Name>/.obsidian/vault_rules.json` to bootstrap a new vault.

4. **Frontmatter Schema Adapter**:
   - Dynamic schema mapping to align with custom vault metadata keys (e.g., matching standard keys like `summary`, `category`, `keywords` to user-customized keys like `abstract`, `folder`, `tags`).

5. **Universal CLI Vault Registry**:
   - An environment configuration/registry map that maps CLI `--folder` arguments to target directories, automatically resolving local paths and dynamic profiles.
