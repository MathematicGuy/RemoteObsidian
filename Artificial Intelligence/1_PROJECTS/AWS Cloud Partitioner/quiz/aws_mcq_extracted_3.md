...
- [ ] B. Offload read-heavy traffic from the primary database instance.
...
- [ ] A. SNS Topic with Email and Lambda as subscribers
...
- [ ] A. Configure the database in Multi-AZ deployment
...
- [ ] C. Aurora automatically scales storage up to large capacity
...
- [ ] C. It is a logical subdivision of a VPC within a specific AZ
- [ ] D. It replaces VPC
...
- [ ] C. Enable cross-zone load balancing on the ALB and ensure there are healthy targets in multiple Availability Zones.
...
- [ ] B. Deploy the RDS instance in a private subnet, use a NAT gateway to allow outbound connections for software updates, and configure a security group to allow traffic only from the web server.
...
- [ ] D. It has a route in its route table that points to an Internet Gateway.
...
- [ ] B. To allow communication between your VPC and the internet.
...
- [ ] D. Enable Multi-AZ on the primary instance and create one or more Read Replicas to handle the read traffic.
...
- [ ] B. Use the AWS Schema Conversion Tool (AWS SCT) to convert the schema and stored objects, then use AWS DMS to migrate the data.
...
- [ ] A. The Visibility Timeout of the SQS queue is set lower than the time required to process the message.
...
- [ ] C. It has a direct route to the internet via an Internet Gateway.
...
- [ ] C. A subnet is a slice of your VPC's IP range that lives in a specific Availability Zone.
...
- [ ] C. They operate at the instance level and are stateful.
...
- [ ] D. They allow all outbound traffic.
...
- [ ] B. Aurora is AWS's proprietary high-performance relational database compatible with MySQL and PostgreSQL
...
- [ ] A. Security Groups support "Allow" rules only; NACLs support both "Allow" and "Deny" rules
...
- [ ] C. HTTP traffic from the Security Group ID of the ALB
...
- [ ] B. The Route Table associated with the Private Subnet does not have a route pointing 0.0.0.0/0 to the NAT Gateway.
...
- [ ] B. The NAT Gateway is located in the private subnet
...
- [ ] A. No requirement of NAT Gateway Usage for internet access
...
- [ ] C. A subnet must reside in one Availability Zone
...
- [ ] A. Choosing IP address ranges and subnets
...
- [ ] B. Cloning uses a copy-on-write protocol, sharing the same storage volume initially, making it much faster and cost-efficient
...
- [ ] B. Create a Read Replica for the analytics workload, and enable Multi-AZ deployment for high availability
...
- [ ] B. Amazon RDS for managing relational database instances.
...
- [ ] C. The message enters a visibility timeout period where it is hidden from other consumers.
...
- [ ] D. Create Read Replicas and redirect read queries there.
...
- [ ] C. Read Replicas uses asynchronous replication (ASYNC), while Multi-AZ uses synchronous replication (SYNC).
...
- [ ] B. Use an SNS topic to publish the event, with multiple SQS queues subscribed to that topic.
...
- [ ] C. A message may be delivered more than once
...
- [ ] B. Provide disaster recovery and high availability
...
- [ ] B. Infrequent or unpredictable workloads
...
- [ ] B. At-least-once delivery with best-effort ordering
...
- [ ] B. Synchronous replication to ensure data consistency
...
- [ ] D. They are stateful, meaning they automatically allow return traffic.
...
- [ ] B. Use a NAT Gateway in a public subnet and update the route table
...
- [ ] A. A Public Subnet has a route table connected directly to an Internet Gateway, while a Private Subnet does not.
...
- [ ] B. It allows for the "decoupling" of application components to improve scalability.
...
- [ ] B. Create an RDS Read Replica and direct analytics queries to it
...
- [ ] B. Place web servers in a public subnet and database servers in a private subnet within the same VPC
